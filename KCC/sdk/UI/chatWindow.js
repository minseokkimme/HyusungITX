                ...
                if (messageHtml === '' && msgData && msgData.message && msgData.message[0]) {
                    ...
                    if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "button") {
                        ...
                        setTimeout(function () {                                                        
                            ...
                            '<button onclick="delteAddYoutube()">X</button> <iframe width="653" height="392" src="https://www.youtube.com/embed/NyyAdZKEzTY?start=61&end=71&autoplay=1" title="한미 아모디핀 스크립트" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';

                            var test = document.getElementsByClassName("tablechartDiv regular")[0].outerHTML.replaceAll("&lt;br /&gt;","<br>");
                            document.getElementsByClassName("tablechartDiv regular")[0].outerHTML = test;
                                        ...
                                        {{if tableRow.Values.length>1}}\
                                            <tr {{if key > 4}}class="hide"{{/if}}>\
                                                {{each(cellkey, cellValue) tableRow.Values}} \
                                                    <td  {{if cellkey === tableRow.Values.length-1}}colspan="2"{{/if}} class=" {{if key == 0}} addTopBorder {{/if}}" {{if msgData.message[0].component.payload.columns[cellkey][1]}}style="text-align:${msgData.message[0].component.payload.columns[cellkey][1]};" {{/if}} title="${cellValue}">{{html helpers.convertMDtoHTML(cellValue, "bot")}}</td>\
                                                {{/each}} \
                                            </tr>\
                                        {{/if}}\
                                    {{/each}} \
