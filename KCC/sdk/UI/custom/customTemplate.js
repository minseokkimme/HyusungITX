				...
				var showFullTableModal = document.getElementsByClassName("showMore");
					for (var i = 0; i < showFullTableModal.length; i++) {					
						...
						showFullTableModal[i].onclick = function () {
							...
							// var code = '';
							// console.log(msgData.message[0].component.payload.names);
							// for(var z = 0; z < msgData.message[0].component.payload.names.length; z++){
							// 	 code = code + '<img src="libs/images/'+ msgData.message[0].component.payload.names[z] + '"></img><br></br>';
							// 	 console.log(code);
							// }
							// console.log(code);
							// document.getElementsByClassName("largePreviewContent")[0].innerHTML = code;
							$(parentli).find('.newImage').clone().appendTo(".largePreviewContent").css("display","block");
							...
							span.onclick = function () {
								...
								// document.getElementsByClassName("largePreviewContent")[0].innerHTML = "";
...
var advancedListTemplate = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
	...
	var manyColorTemplate = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
                ...
                {{/if}} \
						...
						var showScript = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
						{{if msgData.message}} \
								<li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId || msgItem.clientMessageId}"\
										...
										<div class="buttonTmplContent"> \
												...
												<ul class="buttonTmplContentBox">\
														<div class="newImage" style="display:none;"> \
														...
														{{each(key, msgItem) msgData.message[0].component.payload.buttons}}\
																<a href=""#>\
																		<li {{if msgData}}msgData="${JSON.stringify(msgData)}"{{/if}} {{if msgItem.url}}url="${msgItem.url}"{{/if}} class="buttonTmplContentChild showMore" data-value="${msgItem.value}" type="${msgItem.type}">\
																				${msgItem.title}\
																		</li> \
																</a> \
														{{/each}} \
